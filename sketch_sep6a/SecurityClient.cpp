...
SecurityClient::SecurityClient(const std::string& mdns_hostname, const std::string& service_type, const std::string& protocol)
  : mdns_hostname(mdns_hostname), service_type(service_type), protocol(protocol) {
}

std::optional<IPAddress> SecurityClient::findServiceIP() {
...
void SecurityClient::connectToService(const String& ip) {
  if (!connected) {
...
bool SecurityClient::sendPostPlainRequest(const String& endpoint, const String& data) {
...
void SecurityClient::closeConnection() {
  if (connected) {
    client.close();
    connected = false;
